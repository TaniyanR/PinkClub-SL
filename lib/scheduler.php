...
function scheduler_run_schedule(array $schedule): array
{
    ...
    return match ($type) {
        'items' => scheduler_run_items_schedule(sokumiru_sync_service('items'), $settings),
        default => ['synced_count' => 0, 'message' => '未対応スケジュールです'],
    };
}

function scheduler_run_items_schedule(SokumiruSyncService $service, array $settings): array
{
