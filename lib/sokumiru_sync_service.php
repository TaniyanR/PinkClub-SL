...
require_once __DIR__ . '/sokumiru_normalizer.php';
require_once __DIR__ . '/repository.php';

class SokumiruSyncService
{
    public function __construct(private readonly SokumiruApiClient $client, private readonly PDO $pdo)
    {
        $this->ensureSchema();
    }
    ...
    public function syncItemsBatch(string $siteCode, string $serviceCode, string $floorCode, int $batch, int $offset = 1, array $extraParams = [], array $excludeKeywords = []): array
    {
        ...
        $fetchAndSave = function (int $requestOffset, bool $advancePastOffset) use (
            ...
            $fetchedItems = SokumiruNormalizer::normalizeItemsResponse($response);
    ...
    private function upsertItem(array $item): int
    {
        ...
        $this->pdo->prepare($sql)->execute([
            'content_id' => $item['content_id'], 'product_id' => $item['product_id'], 'item_source' => 'sokumiru_product', 'title' => $item['title'],
